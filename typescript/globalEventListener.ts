...
document.addEventListener('keydown', function (event: KeyboardEvent) {
  ...
  if (event.key.toLocaleLowerCase() === 'z') isZPressed = true;
...
function update() {
  if (isArrowRightPressed) Cuphead.getInstance().moveRight();
  if (isArrowLeftPressed) Cuphead.getInstance().moveLeft();
  if (isZPressed) Cuphead.getInstance().jump();
  if (isArrowDownPressed) Cuphead.getInstance().duck();
